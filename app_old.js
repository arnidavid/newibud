...
function renderListings(ls, rows) {
  ...
  ls.sort((a, b) => (o[a._s.s] ?? 1) - (o[b._s.s] ?? 1) || (a.fm_thkr || 999) - (b.fm_thkr || 999));
...
function addrKey(addr) {
  if (!addr) return '';
  // Match: one or more words (street name) + number (optional letter suffix)
...
function parseSheetPrice(priceStr) {
  // "44.900.000 kr." → 44900000, "Tilboð" → 0
...
async function renderRecentSales(postnr) {
  ...
  try {
    ...
    // Filter valid sales (outlier filter)
    ...
    // Build lookup from sheets: addrKey → sheet row (keep latest if duplicates)
    const sheetLookup = {};
    for (const sr of sheetRows) {
      const key = addrKey(sr.titill);
      if (key && !sheetLookup[key]) {
        sheetLookup[key] = sr;
      }
    }
    ...
    for (const sale of validSales) {
      ...
      const saleDateRaw = sale.thinglystdags;
      const saleDate = new Date(saleDateRaw).toLocaleDateString('is-IS');
      const salePrice = sale.kaupverd * 1000; // kaupskrá is in thousands
      const saleSqm = sale.einflm;
      const saleFm = Math.round(sale.kaupverd / sale.einflm); // already in þ.kr/m²

      // Match with sheet listing using addrKey
      ...
      const adFm = adSqm > 0 && adPrice > 0 ? Math.round(adPrice / adSqm / 1000) : 0; // convert to þ.kr/m²
      const adDate = matched ? (matched.skrad || '') : '';

      // Calculate difference
      let diffPct = 0;
      let diffClass = 'ns-nomatch';
      let diffLabel = '';
      if (adPrice > 0 && salePrice > 0) {
        diffPct = Math.round((1 - salePrice / adPrice) * 100);
        if (diffPct > 0) {
          diffClass = 'ns-discount';
          diffLabel = 'afsláttur';
        } else if (diffPct < 0) {
          diffClass = 'ns-premium';
          diffLabel = 'yfirverð';
          diffPct = Math.abs(diffPct);
        } else {
          diffClass = 'ns-nomatch';
          diffLabel = 'jafnt';
        }
      }
      ...
      if (matched && adPrice > 0) {
        ...
        if (adDate) {
          html += `<div style="font-size:.72rem;color:var(--tx3);margin-top:.5rem">Auglýst á fastinn.is: ${adDate}</div>`;
        }
      } else {
